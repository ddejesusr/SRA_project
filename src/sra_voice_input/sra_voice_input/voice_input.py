...
class VoiceInputNode(Node):
    ...
    def __init__(self):
        ...
        self.stop_pub = self.create_publisher(
            String, "/sra/operator/stop_request", 10
        )

        self.recover_pub = self.create_publisher(
            String, "/sra/operator/recover", 10
        )
        ...
        self.STOP_KEYWORDS = {"stop", "parada", "detener", "emergencia", "alto", "para"}

        self.RECOVERY_KEYWORDS = {"reiniciar", "reset", "reanudar", "recuperar", "reinicia"}
    ...
    def transcribe_and_publish(self, frames):
        ...
        try:
            ...
            if any(kw in text.lower() for kw in self.STOP_KEYWORDS):
                self.stop_pub.publish(msg)          # STOP COMMAND, bypasses the gate entirely
            elif any(kw in text.lower() for kw in self.RECOVERY_KEYWORDS):
                self.recover_pub.publish(msg)
            else:
                self.command_pub.publish(msg)       # goes through normal gate
