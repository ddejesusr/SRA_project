...
class TaskManagerNode(Node):
    ...
    def __init__(self):
        ...
        # Local mirror of the state machine's broadcast state.
        # task_manager does not own system state — it only reads it to
        # decide whether it's safe to reserve/dispatch right now.
    ...
    def on_system_state(self, msg: String):
        """Keep a local read-only mirror of the state machine's state."""
        try:
            data = json.loads(msg.data)
            self.current_state = data.get("state")
        except Exception as exc:
            self.get_logger().error(f"Error processing system state: {exc}")
    ...
    def command_callback(self, msg: String):
        ...
        # ----
        # Behavioral gate: task_manager should only act on a valid command
        # while the state machine is in PARSING or RESERVING.
        #
        # Why both, not just RESERVING: command_parser and the state
        # machine both subscribe to /sra/ai/command independently, so
        # delivery order between "task_manager sees the command" and
        # "state machine transitions PARSING → RESERVING" is not
        # guaranteed. Requiring RESERVING only would create a race where
        # task_manager could reject a perfectly valid command just
        # because the state machine hadn't updated yet. PARSING is
        # accepted too since it's the state we're guaranteed to still be
        # in at the moment this message was published.
        #
        # As with command_parser, we fail OPEN (with a warning) if no
        # state has been received yet, so the system isn't deadlocked
        # before the state machine's first broadcast arrives.
        # ----
        if self.current_state is not None and self.current_state not in ("PARSING", "RESERVING"):
            self.get_logger().warn(
                f"Ignoring command: system is {self.current_state}, "
                f"expected PARSING or RESERVING"
            )
            self.publish_alert(
                "warning",
                f"Comando ignorado por task_manager: sistema en "
                f"{self.current_state}, se esperaba PARSING o RESERVING"
            )
            return
        if self.current_state is None:
            self.get_logger().warn(
                "No system state received yet — proceeding anyway (fail-open)"
            )
