...
# Minimum LLM confidence to act on a delivery intent.
# Below this, the system asks the operator to repeat rather than
# committing an inventory reservation on an uncertain parse.
# Tune this against real transcriptions from the work area.
...
- All the text sent as "response" field must be in language (Spanish → "es").
...
Respond ONLY with this JSON — no other text:
{
  ...
  "response":    "<spoken response for the operator in spanish>",
  "language":    "es",
...
class CommandParserNode(Node):
    ...
    def __init__(self):
        ...
        self.stop_pub = self.create_publisher(String, "/sra/system/emergency_stop", 10)
    ...
    # -----------------------------------------------------------------------
    # Stop Command — no state check, no LLM call needed:
    def stop_request_callback(self, msg: String) -> None:
        """
        Dedicated path for emergency stop. Bypasses the behavioral gate
        and the LLM entirely — stop is unambiguous and time-critical.
        Publishes directly to /sra/system/emergency_stop.
        """
        self.get_logger().warn(
            f"Stop request received: \"{msg.data}\" — publishing emergency stop."
        )
        stop_msg = String()
        stop_msg.data = json.dumps({
            "source":    "voice",
        ...
        self._publish_tts("Parada de emergencia activada.")
    ...
    def _validate_delivery(self, parsed: dict) -> dict:
        ...
        # Confidence is clamp to valid range
        conf = parsed.get("confidence", 0.0)
        if not isinstance(conf, (int, float)) or not (0.0 <= conf <= 1.0):
            return {**parsed, "confidence": 0.0}

        # Confidence gate — reject uncertain parses before they reach the queue
        if conf < CONFIDENCE_THRESHOLD:
            return {
                "error": (
                    f"Low confidence parse ({conf:.2f} < {CONFIDENCE_THRESHOLD}). "
                    f"Please repeat the command more clearly."
                ),
                "intent": "delivery",
                "confidence": conf,
            }
