...
class StateMachineNode(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            String, "/sra/operator/raw_command", self.on_raw_command, 10
        )
        self.create_subscription(
            String, "/sra/ai/parse_result", self.on_ai_command, 10
        )
        self.create_subscription(
    ...
    def _on_timeout(self):
        ...
        self.get_logger().warn(f"Timeout in state {timed_out_state}")
        self.transition(
            "ERROR",
            error_detail=f"Timeout while in {timed_out_state}"
        )
