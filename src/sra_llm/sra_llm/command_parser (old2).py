...
class CommandParserNode(Node):
    ...
    def __init__(self):
        ...
        # We do NOT own or decide system state here — we only read it to
        # gate behavior. None means "no state message received yet".
        ...
        # Subscriber: system state broadcast from sra_state_machine
    ...
    def command_callback(self, msg: String):
        ...
        # ----
        # Behavioral gate: only accept a NEW command while the system is
        # IDLE. This is the frontline defence against a second delivery
        # intent racing with one already in flight (RESERVING/DELIVERING/
        # COMPLETING/ERROR). If we haven't heard from the state machine
        # yet (current_state is None), we fail OPEN with a warning rather
        # than silently dropping the very first command after boot.
        # ----
        if self.current_state is not None and self.current_state != "IDLE":
            self.get_logger().warn(
                f'Ignoring command while system is {self.current_state} '
                f'(not IDLE): "{raw_text}"'
            )
            self._publish_alert(
                "warning",
                f"Comando ignorado: el sistema está en {self.current_state}, "
                f"no en IDLE"
            )
            return
